...
program define mapsm, rclass
	version 16.0
	syntax varlist(min=1 max=10 numeric) [, GRoup(varlist fv) Seed(int 1234) Name(string) SIze(int 10) SMD(string asis) ITerate(int 0) REPLACE NOTABle LOG]
	
	**Check the arms are equal to the predicted probability from the propensity score model**
	
	di `: word count `varlist''
	qui tab `group'
	scalar n_group=r(r)
	...
	**Iteration**
	...
	if `iterate'>0{
		...
		forvalues h=`=scalar(seed)'/`=scalar(seed_final)'{
			...
			**Tabulation should be quite if iterate**
			...
			if "`smd'"!="" & `=scalar(arms)'==1 {
				**Show iteration log**
				if "`log'"!=""{
					di in green "SMD Mean at seed `h'= " in yellow `=scalar(std_represent)'
				}
			}
			else if "`smd'"!="" & `=scalar(arms)'>1{
				**Show iteration log**
				if "`log'"!=""{
					di in green "SMD Max at seed `h'= " in yellow `=scalar(std_represent)'
				}
			}
		}		
		...
		**Extract maximum SMD_max/SMD and report the iteration seed**
		...
		di in green "The fittest SMD " _col(28)"= " in yellow _col(29) %6.4f `=scalar(fittest_value)'
		forvalues m=1/`iterate'{
			if `=scalar(fittest_value)'==I[`m',2]{
				scalar fittest_seed=`m'+`=scalar(seed)'
			}
		}
		di in green "The fittest seeding number = " in yellow _col(25) `=scalar(fittest_seed)'
		return matrix I I
	}
	else if `iterate'==0{
...
return scalar std_represent=r(fittest)
return scalar fittest_seed=r(seed)
return scalar size=r(size)
