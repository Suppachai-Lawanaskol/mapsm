...
property, such as standardized mean difference.
 
{p 10 5 3}
The best seed-setting number (among those selected)
...
{p 10 5 3}
After actual matching with the append option, the original dataset (pre-match cohort) will be placed next to the last observation of the post-match cohort. The similarity between treatment arms can be visualized using a kernel density plot of the propensity score, especially when there are more than two treatment arms. 

{p 10 5 3}
By design, love plots and histograms are not supported for visualization in this context. To illustrate this, an example command has been provided in the example section of this help file.

{marker options}{...}
{title:Options}

{p2colset 10 30 31 0}{...}
{p2col:{opt g:roup(varname)}} Specify the treatment arms. Must be specified. {p_end}

{p2col:{opt se:ed(integer)}} Specify the initial seeding number. Default is 1234. Commonly use to re-specify after dryrun of imaginary matched cohort.{p_end}

{p2col:{opt n:ame(string)}} Specify the strata variable name. Default is strata. {p_end}

{p2col:{opt si:ze(integer)}} Specify the total strata number. Default is ten.{p_end}

{p2col:{opt sm:d(varlist)}} Specify the pre-treatment confounder from propensity score model. Optional.{p_end}

{p2col:{opt it:erate(integer)}} In case of smd option is specified, the iteration round should be 
