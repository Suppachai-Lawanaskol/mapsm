...
{p 5 5 3}In actual matching, we appends the original cohort next to the last observation of matched cohort. Generate the default name {cmd:appnd} variable to specify the cohort type. For the next steps of diagnostic balance checking and illustration, return the {p_end}
