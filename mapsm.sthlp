...
{p 5 5 3}
Import Chocolate cyst example dataset. The surgeon was choosing between the laparoscopic approach (MIS) and laparotomy approach (Open) to diagnose, stage, and eradicate the endometrioma.{p_end}
...
{p 5 5 3}
Create an imaginary matched cohort and record the seeding number. Report the smallest balance diagnostic value and its seeding number. Input the probability variable. Binary treatment group. Specify initiation seeding number to {cmd:1234}. Strata variable name, {cmd:"Block"}. Strata size of {cmd:10}. The iterate round of {cmd:200}. The Covariates accountable for the balance diagnostic are {cmd:Age, Body weight, Body mass index, Bilaterality, and Preoperative endometriotic diameter}. Strata tabulation across the treatment groups is suppressed.{p_end}
...
{p 5 5 3}
After the 200 iterations, the mean standardized difference is {cmd:.011}, and the seeding number is {cmd:1301}. Now, we are matching. Without {cmd:"notable"} option, Pre-match and post-match cohort tabulations will be shown.
...
{p 5 5 3}
Import Coronary artery bypass grafting example dataset. The surgeon was choosing among the Conventional coronary artery bypass grafting (CABG),the Off-pump CABG (OPCAB), and On-pump beating heart CABG (ONBHCAB). The study focuses on comparing the short- and long-term outcomes of different CABG techniques.
...
{p 5 5 3}
Create an imaginary matched cohort and record the seeding number. Report the smallest balance diagnostic value and its seeding number. Input the probability variable. Binary treatment group. Specify initiation seeding number to {cmd:1234}.
 Strata variable name, {cmd:"Block"}. Strata size of {cmd:10}. The iterate round of {cmd:50}. The Covariates accountable for the balance diagnostic are {cmd: Sex, Age, New York Heart Association (NYHA) functional class, Canadian Cardiovascular
 Society (CCS) Angina Grade,  ACEI/ARB, Aspirin, Clopidogrel, Chronic kidney disease stage V, Coronary artery disease type
 (e.g. single, double, triple vessel disease),and Left main disease}. Strata tabulation across the treatment groups is suppressed. {p_end}
...
mapsm cabg opcab onbhcab, group(sxtype) seed(1234) name(block) size(10) smd(sex age i.nyfc i.ccs aceiarb asa clopidogrel ckd5esrd i.cadtype lm) iterate(50) notab: mapsm cabg opcab onbhcab}
...
With {cmd:replace} option, the stata replace the existing strata variable.

{phang2}{stata mapsm cabg opcab onbhcab, group(sxtype) seed(1255) name(block) size(10): mapsm cabg opcab onbhcab, group(sxtype) seed(1255) name(block) size(10)}
