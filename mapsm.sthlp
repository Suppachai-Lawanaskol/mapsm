...
balance diagnostic are {cmd:Age, Body weight, Body mass index, Bilaterality, and 
Preoperative endometriotic diameter}. Strata tabulation across the treatment groups is suppressed.{p_end}
...
Conventional coronary artery bypass grafting (CABG),the Off-pump CABG (OPCAB), and On-pump beating heart CABG (ONBHCAB). The study focuses on comparing the short- and long-term outcomes of different CABG techniques.

{phang2}{stata `"use https://raw.githubusercontent.com/Suppachai-Lawanaskol/mapsm/main/cabg.dta,clear"'
: Download cabg.dta}

{p 5 5 3}
Estimate propensity score with multinomial logistic regression{p_end}

{phang2}{stata mlogit sxtype sex age i.nyfc i.ccs aceiarb asa clopidogrel ckd5esrd i.cadtype lm
: mlogit sxtype sex age i.nyfc i.ccs aceiarb asa clopidogrel ckd5esrd i.cadtype lm}
...
the balance diagnostic are {cmd: Sex, Age, New York Heart Association (NYHA) functional class, 
...
and Left main disease}. Strata tabulation across the treatment groups is suppressed.{p_end}

{phang2}{stata `"mapsm cabg opcab onbhcab, group(sxtype) seed(1234) name(block) size(10) 
smd(sex age i.nyfc i.ccs aceiarb asa clopidogrel ckd5esrd i.cadtype lm) iterate(50) 
notab"': mapsm cabg opcab onbhcab, group(sxtype) seed(1234) name(block) size(10) 
smd(sex age i.nyfc i.ccs aceiarb asa clopidogrel ckd5esrd i.cadtype lm) iterate(50) notab}
...
pre-match and post-match cohort tabulations will be shown. With {cmd:replace} option, 
the stata replace the existing strata variable.

{phang2}{stata mapsm cabg opcab onbhcab, group(sxtype) seed(1255) name(block) size(10) replace: 
mapsm cabg opcab onbhcab, group(sxtype) seed(1255) name(block) size(10) replace}
