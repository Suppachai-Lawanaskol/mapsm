...
{p 10 5 3}
	The mapsm (stands for multiple arms propensity score matching) command for the pairwise multiple propensity score matching. For example, in the pairwise matching of two treatment arms study, the participants will be matched within strata. Each stratum sorts the propensity score by the probability of choosing each treatment arm. Same probability interval in each strata will be matched. Multiple matching processes do in the same manner. We also include the ability to iterate to the fittest diagnostic balance parameters. The pre-treatment confounder set similarity will be measured with the mean standardized difference for the two treatment arms study and with the maximum standardized difference for the more than treatment arms study. 
...
{phang2}{stata `"import delimited "https://raw.githubusercontent.com/Suppachai-Lawanaskol/mapsm/main/chocolate_cyst.csv",clear"': import delimited "https://raw.githubusercontent.com/Suppachai-Lawanaskol/mapsm/main/chocolate_cyst.csv",clear}
